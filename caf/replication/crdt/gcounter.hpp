...
struct gcounter_transaction : public base_transaction {
  ...
  gcounter_transaction(std::string topic, operation_t op, T value)
...
struct gcounter_impl {
  ...
  gcounter_impl() : buffered_{} {
    // nop
  }

  gcounter_impl(const gcounter_impl&) = default;
  ...
  gcounter_impl<T> merge(const gcounter_impl<T>& other) {
    std::unordered_map<node_id, T> delta;
