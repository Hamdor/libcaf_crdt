...
struct gcounter_transaction : public base_transaction {
  ...
  gcounter_transaction(operation_t op, T value)
    : base_transaction("", {}), buffer_{std::make_tuple(node_id{}, op, value)} {
    // nop
  }

  /// Construct a new transaction
  gcounter_transaction(std::string topic, node_id nid)
...
struct gcounter_impl {
  ...
  gcounter_impl<T> apply(const transaction_t& history) {
    if (history.operation() == operator_t::increment_by)
      map_[history.node()] += history.value();
  }
  ...
  gcounter_impl<T> merge(const std::string& topic,
                         const gcounter_impl<T>& other) {
    std::unordered_map<node_id, T> delta;
    for (auto& tup : other.map_) {
      auto key = tup.first;
      if (map_[key] < tup.second) {
        // Generate transaction
        auto diff = tup.seoncd - map_[key];
        buffered_.emplace(topic, key, operator_t::increment_by, diff);
        // Overwrite old value
        map_[key] = tup.second;
        // Store in delta
  ...
  transaction_t get_cmrdt_transactions(const std::string&) const {
    auto cpy = buffered_;
    buffered_.clear();
    return cpy;
  }
...
private:
  std::vector<transaction_t> buffered_; /// Here we buffer transactions,
                                        /// we discovered by merge
...
public:
  ...
  T operator+=(const T& by) {
    if (by > 0) {
      value_ += by;
      publish(transaction_t{operator_t::increment_by, by});
    }
    return value_;
  }
  ...
  T& operator++() {
    value_ += 1;
    publish(transaction_t{operator_t::increment_by, 1});
    return value_;
  }
...
struct gcounter : public cmrdt::gcounter_impl<T> {
  ...
  using internal_type = delta::gcounter_impl<T>;
};
